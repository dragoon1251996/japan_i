...
def LSTM(X,seqlen,weights,biases):
    ...
    #
    # lstm_F=rnn.BasicLSTMCell(n_hidden,forget_bias=1.0,name="lstm_F")
    #
    # lstm_B = rnn.BasicLSTMCell(n_hidden, forget_bias=1.0,name="lstm_B")
    #
...
with tf.Session(config=tf.ConfigProto(log_device_placement=True)) as sess:
    ...
    while  True:
        try:
            ...
            manyOut=sess.run(out, feed_dict={X: inX, Y: inY, seqlen: inS})
            try:
                for j in manyOut:
                    temp = [[j[i], i + 1] for i in range(len(j))]
                ...
                #     if out[i] == max(out):
                #         print(i)
            print(time.time()-thoigian,"***********")
            data = test_data(test,"入")
            Dtest=config_data([data],seq_max_len)
            x = [x[0] for x in Dtest]
            y = [y[1] for y in Dtest]
            s = [s[2] for s in Dtest]
            print(np.shape(x),np.shape(inX))
            print(np.shape(y),np.shape(inY))
            print(np.shape(s),np.shape(inS))
